...
| AgentPresenter     | AI 对话、工具调用、阶段感知 systemPrompt、API Key 验证                                                          |
| ToolPresenter      | 9 tools（read/write/edit/exec/ask_user/open + 3 evolution）                                                     |
| EvolutionPresenter | 进化状态机（idle→discuss→coding→applying）+ CHANGELOG + apply + archive CRUD + AI 语义回滚 + build verification |
...
- API 验证: `agentPresenter.verifyApiKey()` 使用 `generateText` 发送 maxOutputTokens:1 请求
