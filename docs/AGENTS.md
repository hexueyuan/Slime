...
| EvolutionPresenter | 进化状态机（idle→discuss→coding→applying）+ CHANGELOG + apply + getHistory(解析CHANGELOG) |
...
- Agent 工具: evolution_start / evolution_plan / evolution_complete
- 用户操作（IPC）: cancel / rollback / restart
- CHANGELOG.slime.md 记录进化节点，tag 格式: `egg-v0.1-{user}.{seq}`
- dev 模式重启：UI 按钮提示，用户手动重启
