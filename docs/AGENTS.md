...
Slime 是一个自我进化的 Electron 桌面应用。v0.1 (egg) 验证核心假设：软件可以通过 AI Agent 实现自我迭代进化。v0.2 (brave) 引入内置 LLM Gateway，统一多渠道路由、负载均衡、熔断、协议转换和统计。

## 项目结构

- `src/main/`: Electron 主进程
  - `presenter/`: Presenter 单例 + 子 Presenter（通过 `presenter:call` IPC 分发）
  - `db/`: better-sqlite3 数据库（gateway 表：channels, channel*keys, groups*, group_items, api_keys, model_prices, relay_logs, stats_hourly, stats_daily）
...
| GatewayPresenter   | LLM Gateway 生命周期管理：渠道/分组/API Key/价格 CRUD、Router/Balancer/Circuit/Server init/destroy、Slot 解析、内部密钥      |
...
- 4 步: Welcome → AddChannelStep → SlotMappingStep → IdentityCompleteStep
- AddChannelStep: 选择渠道类型（anthropic/openai/gemini/deepseek/volcengine/custom），输入 baseUrl + API Key，testChannel 验证
- SlotMappingStep: 5 个 Slot 下拉（reasoning auto/lite/pro/max + chat）绑定到分组
...
- **数据层**: better-sqlite3 WAL 模式，8 张表（channels, channel*keys, groups*, group_items, api_keys, model_prices, relay_logs, stats_hourly, stats_daily）
...
- **Model Slot**: 抽象模型选择（category: text/image, tier: chat/reasoning, level: lite/pro/max/auto），auto 模式返回首个匹配分组
- **AgentPresenter 集成**: 通过 `createAnthropic({ baseURL: "http://127.0.0.1:{port}/" })` 连接本地 Gateway，resolveSlot 获取模型名
- **Gateway UI**: GatewayPanel（4 tab: 渠道/分组/接入/日志）+ SettingsDialog Gateway tab（端口/熔断/保留/Slot 映射）
