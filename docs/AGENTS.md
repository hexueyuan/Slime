...
- `src/main/`: Electron 主进程
  ...
  - `db/`: better-sqlite3 数据库（gateway 表：channels, channel*keys, groups*, group_items, api_keys, model_prices, models, relay_logs, stats_hourly, stats_daily）
...
- CapabilityTagStep: 为渠道模型标记能力标签（reasoning/chat/vision/image_gen）
...
- **Capability Selection**: 基于能力的模型选择（capabilities: reasoning/chat/vision/image_gen），`select(requirements)` 按 CapabilityRequirement 匹配 models 表中的注册模型
