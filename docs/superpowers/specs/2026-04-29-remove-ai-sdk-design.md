...
**1. SSE 解析器测试 (core/__tests__/sseParser.test.ts)**
```typescript
describe('parseSSE', () => {
...
**2. Anthropic 流解析测试 (providers/anthropic/__tests__/streamParser.test.ts)**
```typescript
...
**3. 请求构建测试 (providers/anthropic/__tests__/requestBuilder.test.ts)**
```typescript
...
export function createLLMClient(provider: string, config: Config): LLMClient {
  switch (provider) {
    ...
    case 'gemini': return new GeminiClient(config.baseURL, config.apiKey)
