...
export function getCallerContext(): CallerContext {
  ...
  if (!dataDir) throw new Error('SLIME_DATA_DIR is not set. Run via Slime app or slime-cli wrapper.')
...
import { readLogs, clearLogs, getTodayLogPath, formatLogLine } from '../../../src/cli/utils/logReader'
...
describe('formatLogLine', () => {
  it('formats valid JSON log line', () => {
    const line = JSON.stringify({ timestamp: '2026-05-05T10:00:00.000Z', level: 'info', message: 'started', port: 3000 })
  ...
  it('pads level to 5 chars', () => {
    const line = JSON.stringify({ timestamp: '2026-05-05T10:00:00.000Z', level: 'warn', message: 'msg' })
    const formatted = formatLogLine(line)
...
describe('runLogs', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(process, 'exit').mockImplementation((code?: number) => { exitCode = code; throw new Error('exit') })
  })
  ...
  it('passes key to readLogs', async () => {
    ...
    expect(readLogs).toHaveBeenCalledWith('/tmp/slime-data', expect.objectContaining({ key: 'error' }))
  })
...
function parseLogsArgs(args: string[]): { key?: string; tail?: number; head?: number; clear?: boolean } {
...
- [ ] **Step 2: 手动验证入口可被 Node 执行（无 Electron 环境，先跳过打包）**
...
- [ ] **Step 3: 修改 package.json 增加 build:cli 脚本**
...
describe('ToolPresenter exec env injection', () => {
  ...
  beforeEach(() => {
    ...
    const mockBrowserSession = {
      navigate: vi.fn(), screenshot: vi.fn(), snapshot: vi.fn(), click: vi.fn(),
      type: vi.fn(), scroll: vi.fn(), evaluate: vi.fn(), wait: vi.fn(),
      close: vi.fn(), isActive: vi.fn().mockReturnValue(false),
    } as any
...
- [ ] **Step 1: 在 agentChatPresenter.ts 的 chat() 方法中找到 agent 读取位置**
...
- [ ] **Step 2: 在该行之后插入 setSessionContext 调用**
...
- [ ] **Step 2: 修改 index.ts 在 bootstrap 阶段调用 setupCliWrapper**
...
const userName =
  ((await presenter.configPresenter.get('app.userProfile')) as any)?.name ||
  userInfo().username
...
- [ ] **Step 4: 运行 dev 模式手动验证 wrapper 生成**
