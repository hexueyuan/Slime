# browser_* + web_fetch 工具集 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 为 AgentChatPresenter 新增 9 个 browser_* 工具（headless Chromium）和 1 个 web_fetch 工具，共 10 个新工具，总工具数从 9 增至 19。
...
export class BrowserSession {
  ...
  async type(opts: TypeOpts): Promise<void> {
    ...
    const loc = opts.ref
      ...
      : opts.selector
        ...
        : null
    if (!loc) throw new Error(`type requires ref or selector${opts.ref ? ` (unknown ref: ${opts.ref})` : ''}`)
  ...
  async scroll(opts: ScrollOpts): Promise<void> {
    ...
    const dy =
      opts.direction === 'down' ? opts.amount : opts.direction === 'up' ? -opts.amount : 0
    if (opts.ref || opts.selector) {
      const loc = opts.ref
        ? this.refMap.get(opts.ref)
        : page.locator(opts.selector!).first()
      if (!loc) throw new Error(`Unknown ref: ${opts.ref}`)
      await (loc as any).evaluate((el: Element, args: { dx: number; dy: number }) => el.scrollBy(args.dx, args.dy), { dx, dy })
    } else {
...
beforeEach(() => {
  ...
  const mockBrowserSession = {
    ...
    screenshot: vi.fn().mockResolvedValue({ base64: '', mimeType: 'image/png', width: 0, height: 0 }),
