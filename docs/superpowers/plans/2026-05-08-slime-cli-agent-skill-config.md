...
app.put<{ Params: { key: string }; Body: { value: unknown } }>("/config/:key", async (req, reply) => {
  const { key } = req.params
  if (!CONFIG_WRITABLE_KEYS.includes(key)) {
    return reply.status(403).send({ error: `key '${key}' is not writable. Allowed: ${CONFIG_WRITABLE_KEYS.join(', ')}` })
  }
...
async function runAsync(args: string[]): Promise<void> {
  ...
  if (sub === 'list') {
    ...
    const result = (await httpPut(`/config/${rest[0]}`, { value })) as { key: string; value: unknown }
    process.stdout.write(`${result.key}=${JSON.stringify(result.value)}\n`)
  } else {
    throw new Error(
      `未知子命令: ${sub}\n\n可用子命令: list | get | set\n\n运行 \`slime-cli help config\` 查看完整用法说明。`
