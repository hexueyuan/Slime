...
function serializeTask(task: Task): string {
  ...
  const emoji = emojiForStatus(task.status === 'archived' ? (task.completedAt ? 'done' : 'cancelled') as TaskStatus : task.status)
...
export class TaskManager {
  ...
  private async writeAll(main: Task[], archived: Task[]): Promise<void> {
    ...
    const archiveLines =
      archived.length > 0
        ? ARCHIVE_HEADER + archived.map(serializeTask).join('\n') + '\n'
        : ''
  ...
  async list(status?: TaskStatus): Promise<Task[]> {
    ...
    const all = status === 'archived' ? archived : status ? main.filter((t) => t.status === status) : main
    return all
  }
}
```
...
function formatTask(t: Task): string {
  const parts = [`[${t.id}] ${t.description} [${STATUS_LABEL[t.status] ?? t.status}]`, `created:${t.createdAt}`]
